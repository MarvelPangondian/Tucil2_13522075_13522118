...
def showGraph(control_points,depth):
    ...
    plt.plot(*zip(*control_points), marker='o', markersize=4, linestyle='--', color='red', label='Control Points')
    plt.scatter(*zip(*control_points), color='red', zorder=5)
    ...
    annotation_interval = len(curve_points) // 10 or 1 # avoid showing too many points
...
depth = 4
showGraph(control_points,depth)
